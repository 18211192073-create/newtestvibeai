...
DEFAULT_ENGLISH_AUTHORITY_PATTERNS = [
    "openai",
    "anthropic",
    "google deepmind",
    "deepmind",
...
DEFAULT_CHINESE_AI_TECH_PATTERNS = [
    "机器之心",
    "量子位",
    "极客公园",
    "智东西",
...
DEFAULT_HOTLIST_TREND_PATTERNS = [
    "wallstreetcn-hot",
    "zhihu",
    "weibo",
    "douyin",
...
SOURCE_MIX_BUCKETS = ("english_authority", "chinese_ai_tech", "hotlist_trend")
...
def load_assistant_settings(path: str = "config/assistant.yaml") -> Dict[str, Any]:
    ...
    merged["source_mix"] = default["source_mix"] | (data.get("source_mix", {}) or {})
    return merged
...
def _source_priority_boost(source_name: str, source_id: str, source_priority: Optional[Dict[str, Any]] = None) -> float:
    ...
    priority_rules: List[Tuple[str, float]] = list(SOURCE_PRIORITY_RULES)
    for pattern, score in (source_priority.get("boosts", {}) or {}).items():
        priority_rules.append((str(pattern), float(score)))
    for pattern, score in (source_priority.get("downweights", {}) or {}).items():
        priority_rules.append((str(pattern), float(score)))

    positive_boost = 0.0
    negative_boost = 0.0
    for pattern, score in priority_rules:
        if pattern and pattern in normalized:
            score_f = float(score)
            if score_f >= 0:
                positive_boost = max(positive_boost, score_f)
            else:
                negative_boost = min(negative_boost, score_f)
    return max(-30.0, min(30.0, positive_boost + negative_boost))
...
def _categorize_source_kind(candidate: DigestCandidate, mix_cfg: Dict[str, Any]) -> str:
    source_id = str(candidate.raw.get("source_id", "") or "")
    source_blob = f"{candidate.source_name} {source_id} {candidate.source_url}".lower()
    title = candidate.title or ""

    if _match_any_pattern(source_blob, mix_cfg.get("english_patterns", [])):
        return "english_authority"

    if candidate.source_type == "hotlist" or _match_any_pattern(source_blob, mix_cfg.get("hotlist_patterns", [])):
        return "hotlist_trend"

    if _match_any_pattern(source_blob, mix_cfg.get("chinese_patterns", [])):
        return "chinese_ai_tech"

    if _contains_cjk(candidate.source_name) or _contains_cjk(title):
        return "chinese_ai_tech"
...
def _compute_mix_targets(total: int, mix_cfg: Dict[str, Any]) -> Dict[str, int]:
    if total <= 0:
    ...
    used = 0
    for bucket in SOURCE_MIX_BUCKETS:
        raw_target = normalized[bucket] * total
        base = int(raw_target)
        targets[bucket] = base
        decimals.append((raw_target - base, bucket))
        used += base

    remainder = total - used
    for _, bucket in sorted(decimals, key=lambda item: (item[0], normalized[item[1]]), reverse=True):
        if remainder <= 0:
            break
        targets[bucket] += 1
        remainder -= 1
...
def _apply_source_mix(
    ...
    mix_cfg = _source_mix_config(assistant_settings)
    if not mix_cfg.get("enabled", True):
        return candidates[: max(0, target_count)]

    limit = min(max(0, int(target_count or 0)), len(candidates))
    if limit <= 0:
        return []

    targets = _compute_mix_targets(limit, mix_cfg)
    bucketed: Dict[str, List[DigestCandidate]] = {bucket: [] for bucket in SOURCE_MIX_BUCKETS}
    others: List[DigestCandidate] = []
    for candidate in candidates:
        bucket = _categorize_source_kind(candidate, mix_cfg)
    ...
    selected_ids = set()
    selected_bucket_counts = {bucket: 0 for bucket in SOURCE_MIX_BUCKETS}
    for bucket in SOURCE_MIX_BUCKETS:
        quota = targets.get(bucket, 0)
        if quota <= 0:
            continue
        for candidate in bucketed[bucket]:
            if candidate.item_id in selected_ids:
                continue
            selected.append(candidate)
            selected_ids.add(candidate.item_id)
            selected_bucket_counts[bucket] += 1
            if selected_bucket_counts[bucket] >= quota:
                break

    for candidate in candidates + others:
...
def collect_candidates(lookback_hours: int = 24, assistant_settings: Optional[Dict[str, Any]] = None) -> List[DigestCandidate]:
    ...
    sorted_candidates = sorted(dedup.values(), key=lambda item: (item.importance_hint, item.published_at), reverse=True)
    candidate_limit = int(assistant_settings.get("candidate_limit", 60) or 60)
    return _apply_source_mix(sorted_candidates, candidate_limit, assistant_settings=assistant_settings)
