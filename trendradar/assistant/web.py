...
class AssistantHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def _ensure_latest_report(self) -> Dict[str, Any]:
        report = self.storage.get_latest_report()
        if report:
            report["bookmarks"] = self.storage.list_bookmarks()
            report["logs"] = self.storage.list_reading_logs()
            return report
        report = build_daily_digest(assistant_settings=self.assistant_settings, storage=self.storage)
        report["bookmarks"] = self.storage.list_bookmarks()
        report["logs"] = self.storage.list_reading_logs()
        return report
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if path == "/api/reports":
            self._send_json({"ok": True, "reports": self.storage.list_reports()})
            return

        if path == "/api/bookmarks":
            self._send_json({"ok": True, "bookmarks": self.storage.list_bookmarks()})
            return

        if path == "/api/logs":
            query = parse_qs(parsed.query)
            item_id = (query.get("item_id") or [None])[0]
            self._send_json({"ok": True, "logs": self.storage.list_reading_logs(item_id=item_id)})
            return
        ...
        if path == "/api/item":
            ...
            if not item:
                ...
                return
            bookmark = self.storage.get_bookmark(item_id)
            logs = self.storage.list_reading_logs(item_id=item_id)
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if parsed.path == "/api/bookmarks":
            ...
            if bookmarked is False:
                result = self.storage.remove_bookmark(item["item_id"])
            elif bookmarked is True:
                result = self.storage.set_bookmark(item, note=note)
            else:
                existing = self.storage.get_bookmark(item["item_id"])
                if existing:
                    result = self.storage.remove_bookmark(item["item_id"])
                else:
                    result = self.storage.set_bookmark(item, note=note)
        ...
        if parsed.path == "/api/logs":
            ...
            result = self.storage.add_reading_log(item, log_text, log_title=log_title)
            self._send_json({"ok": True, **result})
            return
        ...
        if parsed.path == "/api/log-draft":
            ...
            draft = build_reading_log_draft(
                item=item,
                report=report,
                existing_logs=self.storage.list_reading_logs(item_id=item_id),
